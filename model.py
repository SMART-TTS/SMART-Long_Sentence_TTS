    ...
    def forward(self, attention_hidden_state, memory, processed_memory,
        ...
        if memory.size(0) == 1:  # batch = 1 일때, (합성할때)
            if memory.size(1) < 300:  # 짧은문장
                ...
                # a = [memory.size(1), mel_iter//6 + 100]
                # b = [memory.size(1), mel_iter//6 - 100]
                ...
                # A = memory.size(1) * 4 // 3   # 뭐지,,,,3분의2배정도임
                ...
                if mel_iter < A:
                    ...
                    sw_mask = get_mask_from_lengths2(c)[1:]
...
class Decoder(nn.Module):
    def __init__(self, hparams):
        ...
        # self.prenet = Prenet(hparams.n_mel_channels, hparams.prenet_dim * 2, hparams.prenet_dim, p=0.2)
...
class Tacotron2(nn.Module):
    ...
    def inference(self, inputs):
        ...
        #     encoder_outputs)
        mel_outputs, gate_outputs, alignments = self.decoder.inference(
            encoder_outputs, duration.size(1))
        ...
        if length_regulator_output == None:
            ...
            outputs = self.parse_output_duration(
                [mel_outputs, mel_outputs_postnet, gate_outputs, alignments, length_regulator_output, duration])
